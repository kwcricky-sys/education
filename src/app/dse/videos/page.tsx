...
export default function VideoLibraryPage() {
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/80 px-6 py-12 sm:px-10">
        <div
          aria-hidden
      ...
      <section>
        <h2 className="text-lg font-semibold text-zinc-100">按科目進入</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {libraries.map((lib) => (
            <Link
              key={lib.meta.subject}
              href={`/dse/videos/${lib.meta.subject}`}
              className="group rounded-2xl border border-white/10 bg-zinc-900/60 p-5 transition hover:border-cyan-400/40 hover:bg-zinc-900"
            >
              <h3 className="text-base font-semibold text-zinc-100">{lib.meta.name}</h3>
              ...
              <span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-cyan-300 group-hover:text-cyan-200">
                睇片單
      ...
      <section>
        <h2 className="text-lg font-semibold text-zinc-100">全部影片（可按科目、課題篩選）</h2>
      ...
      <section className="rounded-2xl border border-amber-400/20 bg-amber-400/5 p-5">
        <h2 className="text-sm font-semibold text-amber-200">策展說明與版權</h2>
        <ul className="mt-3 space-y-2 text-xs leading-relaxed text-amber-100/80">
          <li>
        ...
        <p className="mt-4 text-xs leading-relaxed text-amber-100/70">{LEGAL_DISCLAIMER}</p>
      </section>
    </div>
  );
}
